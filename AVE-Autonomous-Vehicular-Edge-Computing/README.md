...
每个接收器检查EREQ中包含的速度矢量，并使用针对beacon描述的相同规则计算速度差。 如果速度差太大，则忽略此EREQ。否则，所有单跳接收器都会将EREQ转发给附近的车辆;即，他们成为这个请求者的转发者。此外，对于与接收器兼容的每个作业，将具有作业brief的查询发送到资源管理器。 然后，资源管理器要求相应的后端生成bid。